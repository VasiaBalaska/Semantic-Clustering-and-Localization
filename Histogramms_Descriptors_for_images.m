...
for i = 1:1:length(temp)
    ...
    save('VideoTable.mat', 'VideoTable_fr_300', '-v7.3');
...
disp(sum);
 ...
 accuracy = 1000;
simMatrixSeq_sizeY = size(simMatrix,1);
simMatrixSeq_sizeX = size(simMatrix,2);
simMatrixSeq = simMatrix;
window = 9;
count=1;
for i=0:simMatrixSeq_sizeY-1
    for j=(i+1):simMatrixSeq_sizeX-1
        indY(count) = i;
        indX(count) = j;
        simVectorSeq(count) = simMatrixSeq(i+1,j+1);
        count = count + 1;
    end
end
simVectorSeq = int32(simVectorSeq*accuracy);
storeFormat = [indY' indX' simVectorSeq'];
dlmwrite('simVectorSeq.txt', storeFormat, 'delimiter', '\t');
